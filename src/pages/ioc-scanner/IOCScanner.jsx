...
const SEV = {
  critical: {
    ...
    bg: "border-red-800/60 bg-red-950/20",
  },
  high: {
    label: "HIGH",
    icon: ShieldAlert,
    ...
    bg: "border-orange-800/60 bg-orange-950/20",
  },
  medium: {
    label: "MEDIUM",
    icon: AlertTriangle,
    ...
    bg: "border-yellow-800/60 bg-yellow-950/20",
  },
  low: {
    label: "LOW",
    icon: Info,
    ...
    bg: "border-blue-800/60 bg-blue-950/20",
  },
  clean: {
    label: "CLEAN",
    icon: ShieldCheck,
    ...
    bg: "border-green-800/60 bg-green-950/20",
  },
};
...
const TAG_ACTORS = {
  ...
  "known-distributor": "Malware Distributor",
  phishing: "Phishing Campaign",
  rat: "RAT Operator",
  "cobalt-strike": "Cobalt Strike C2",
  "abuse-reported": "Reported for Abuse",
...
function RiskGauge({ score = 0, severity = "clean", size = 130 }) {
  ...
  return (
    <svg viewBox="0 0 120 100" style={{ width: size, height: size * 0.84 }}>
      ...
      <text x={cx} y={cy + 22} textAnchor="middle" fill="#64748b" fontSize="10">
        / 100
      </text>
    </svg>
  );
    ...
    <div className="space-y-1">
      <div className="flex justify-between text-xs">
        <span className="text-brand-muted">
          {label}
          {sub && <span className="text-[10px] opacity-60 ml-1">{sub}</span>}
        </span>
...
function SourceBadge({ source }) {
  ...
  return (
    <span
      className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${cfg[source] ?? cfg.manual}`}
    >
      {source}
    </span>
  );
...
function ActionButtons({
  ...
  const markFP = async () => {
    ...
    try {
      ...
      onUpdate?.({
        is_false_positive: res.data.is_false_positive,
        action_status: res.data.action_status,
      });
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const sendBlock = async () => {
    setLoading("block");
    try {
      await api.post("/ioc/bulk-action", {
        scan_ids: [scanId],
        action: "block",
      });
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <button
        onClick={sendBlock}
        disabled={!!loading || actionStatus === "blocked"}
        className={`${base} ${
...
function ScanDetail({ result: initialResult, onClose }) {
  ...
  const actors = [
    ...new Set((result.tags || []).map((t) => TAG_ACTORS[t]).filter(Boolean)),
  ];
  ...
  const downloadJSON = () => {
    ...
    a.href = url;
    a.download = `threatshot-${result.ioc.replace(/[^a-z0-9]/gi, "_")}.json`;
    a.click();
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <span
                className={`text-xs font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-full border ${sev.badge}`}
              >
                {sev.label} RISK
              </span>
            ...
            <p className="font-mono text-brand-text text-sm break-all">
              {result.ioc}
            </p>
          ...
          <div className="flex gap-1 shrink-0">
            <button
              onClick={() => navigator.clipboard.writeText(result.ioc)}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="col-span-2 sm:col-span-1 flex flex-col items-center justify-center py-4 gap-2">
          ...
          />
          <span
            className={`text-[10px] font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-full border ${sev.badge}`}
          >
            {sev.label}
          </span>
        </Card>
        ...
        <Card className="space-y-3">
          ...
          {vtTotal > 0 ? (
            <>
              <DetBar
                label="Malicious"
                count={vt.malicious ?? 0}
                total={vtTotal}
                color="#ef4444"
              />
              <DetBar
                label="Suspicious"
                count={vt.suspicious ?? 0}
                total={vtTotal}
                color="#f97316"
              />
              <DetBar
                label="Harmless"
                count={vtTotal - (vt.malicious ?? 0) - (vt.suspicious ?? 0)}
                total={vtTotal}
                color="#22c55e"
              />
              <p
                className="text-xl font-bold font-mono pt-1 border-t border-brand-border"
                style={{ color: sev.ring }}
              >
                {vt.malicious ?? 0}
          ...
          {result.ioc_type === "ip" ? (
            abuse.confidence_score !== undefined ? (
              <>
                <DetBar
                  label="Confidence"
                  count={abuse.confidence_score}
                  total={100}
                  color="#f97316"
                />
                {abuse.total_reports > 0 && (
                  <p className="text-xs text-brand-muted">
                    {abuse.total_reports} abuse reports
                  </p>
                )}
                {abuse.isp && (
        ...
        <Card className="border-red-900/40 bg-red-950/10 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {actors.map((a, i) => (
              <span
                key={i}
                className="text-xs bg-red-900/30 border border-red-800/50 text-red-300 px-3 py-1 rounded-lg"
              >
                {a}
              </span>
            ))}
        ...
        <Card className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {result.tags.map((tag) => (
              <span
                key={tag}
                className="text-xs font-mono bg-brand-bg border border-brand-border text-brand-accent px-2.5 py-1 rounded-lg"
              >
                {tag}
              </span>
            ))}
...
function BulkResult({ result: bulkResult }) {
  ...
  const malicious = allItems.filter(
    (i) =>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-5 gap-2">
        {["critical", "high", "medium", "low", "clean"].map((s) => {
          ...
          return (
            <div
              key={s}
              className={`rounded-xl border p-2.5 text-center ${c.badge}`}
            >
              <p className="text-xl sm:text-2xl font-bold font-mono">
                {counts[s] ?? 0}
              </p>
              <p className="text-[9px] sm:text-[10px] uppercase tracking-widest mt-0.5">
                {s}
              </p>
            </div>
          );
      ...
      <div className="flex flex-wrap items-center gap-3 p-3 bg-brand-surface border border-brand-border rounded-xl">
        ...
        <div className="flex gap-2 shrink-0">
          <button
            onClick={sendBlock}
            disabled={!!actionLoading || blocked || malicious.length === 0}
          ...
          <button
            onClick={sendFP}
            disabled={!!actionLoading || fpIds.size === 0}
      ...
      {allItems.filter((i) => i.severity !== "clean").length > 0 && (
        <div>
          <p className="text-xs text-brand-muted mb-2 flex items-center gap-1.5">
            <AlertCircle className="w-3.5 h-3.5" /> Toggle FP on individual IOCs
            before sending for blocking
          </p>
            ...
            {allItems
              ...
              .map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex items-center gap-2 px-3 py-2 transition-colors ${isFP ? "bg-blue-950/20" : ""}`}
                  >
                    <div
                      className={`w-1.5 h-6 rounded-full shrink-0 ${c.bar}`}
                    />
                    <div className="flex-1 min-w-0">
                      <p className="text-brand-text text-xs font-mono truncate">
                        {item.ioc}
                      </p>
                    ...
                    <span
                      className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border shrink-0 ${c.badge}`}
                    >
                      {item.severity}
                    </span>
                    <button
                      onClick={() => toggleFP(item.scan_id)}
  ...
  return (
    <div className="border-b border-brand-border last:border-0">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-brand-text text-sm font-mono truncate">
            {scanData.ioc}
          </p>
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            {scanData.is_false_positive && (
              <span className="text-[9px] font-bold bg-blue-900/30 border border-blue-800 text-blue-300 px-1.5 py-0.5 rounded">
                FP
              </span>
            )}
            {scanData.action_status === "blocked" && (
              <span className="text-[9px] font-bold bg-red-900/30 border border-red-800 text-red-300 px-1.5 py-0.5 rounded">
                BLOCKED
              </span>
            )}
            <span className="text-brand-muted text-xs">
              {formatRelativeTime(scanData.scanned_at)}
            </span>
          </div>
        </div>
        <div className="shrink-0 text-right mr-1">
          <p
            ...
            style={{ color: sev.ring }}
          >
            {Math.round(scanData.risk_score)}
          </p>
          <p className="text-[10px] uppercase font-semibold text-brand-muted">
            {sev.label}
          </p>
        </div>
        {open ? (
...
function OrgDownloadBar() {
  ...
  const download = async (endpoint, format, filename) => {
    ...
    try {
      const res = await api.get(`${endpoint}?format=${format}&days=30`, {
        responseType: "blob",
      });
      ...
      if (e.response?.status === 400)
        alert("Download requires an organisation account.");
    } finally {
      setLoading(null);
    }
  };
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <div className="flex items-center gap-3 p-3 bg-brand-surface border border-brand-border rounded-xl">
        ...
        <div className="flex gap-1.5 shrink-0">
          ...
          >
            {loading === "/ioc/org-history/downloadcsv" ? (
              <Spinner className="w-3 h-3" />
            ) : (
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-medium bg-brand-bg border border-brand-border rounded-lg hover:border-brand-accent2 text-brand-muted transition-colors disabled:opacity-50"
          >
            {loading === "/ioc/org-history/downloadpdf" ? (
              <Spinner className="w-3 h-3" />
            ) : (
      ...
      <div className="flex items-center gap-3 p-3 bg-brand-surface border border-purple-800/40 rounded-xl">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-medium text-brand-text">CERT-IN Report</p>
          <p className="text-[10px] text-brand-muted">Audit compliance</p>
        </div>
        <div className="flex gap-1.5 shrink-0">
          <button
            onClick={() =>
              download("/ioc/cert-in/download", "csv", "cert-in-report.csv")
            }
            disabled={!!loading}
          ...
          >
            {loading === "/ioc/cert-in/downloadcsv" ? (
              <Spinner className="w-3 h-3" />
            ) : (
              <FileText className="w-3 h-3" />
            )}{" "}
          ...
          <button
            onClick={() =>
              download("/ioc/cert-in/download", "pdf", "cert-in-report.pdf")
            }
            disabled={!!loading}
            className="flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-medium bg-purple-900/20 border border-purple-800/60 rounded-lg hover:bg-purple-900/40 text-purple-300 transition-colors disabled:opacity-50"
          >
            {loading === "/ioc/cert-in/downloadpdf" ? (
              <Spinner className="w-3 h-3" />
            ) : (
              <Download className="w-3 h-3" />
            )}{" "}
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-brand-text">
            IOC Intelligence Scanner
          </h1>
          <p className="text-brand-muted text-xs sm:text-sm mt-0.5">
            Real-time analysis from top intelligence platforms : Block, tag &
            report IOCs for compliance
          </p>
        </div>
            ...
            <Card className="space-y-3">
              <form onSubmit={handleScan} className="space-y-3">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <p className="text-xs text-brand-muted ml-auto hidden sm:block">
        ...
        {historyItems.length > 0 && (
          <Card className="p-0 overflow-hidden">
            <div className="px-4 py-3 border-b border-brand-border flex items-center justify-between">
              ...
              <p className="text-xs text-brand-muted">
                Tap any row to expand full analysis
              </p>
            </div>
            <div>
